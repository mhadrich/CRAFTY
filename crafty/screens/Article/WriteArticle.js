...
const WriteArticle = () => {
  return (
    <View className=" h-screen gap-4 p-4 items-center">
      <Text className="text-lg font-semibold leading-snug">
        Give your article a title
      </Text>
      <TextInput
        className="mb-4 w-11/12 h-16 pl-3 bg-white rounded-md shadow"
        placeholder={"Title"}
      />
      <View className="w-11/12 h-24 items-center justify-center bg-white rounded shadow">
      ...
      <TextInput
        ...
        multiline={true}
        numberOfLines={20}
        className="p-6 w-11/12 h-96 bg-white rounded-lg shadow"
      />
      <TouchableOpacity
        onPress={() => {
          close(false);
